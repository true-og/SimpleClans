...
public class InviteFrame extends SCFrame {
    ...
    private SCComponent createPlayerComponent(@NotNull Player player, int slot) {
        ...
        if (price != 0)
            lore.add(lang("gui.invite.player.price.lore", getViewer(), CurrencyFormat.format(price)));
        lore.add(lang("gui.invite.player.lore", getViewer()));

        SCComponent c = new SCComponentImpl(lang("gui.invite.player.title", getViewer(), player.getName()), lore,
