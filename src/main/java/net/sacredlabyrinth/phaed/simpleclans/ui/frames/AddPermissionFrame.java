...
public class AddPermissionFrame extends SCFrame {
    ...
    public void createComponents() {
        ...
        for (int i = paginator.getMinIndex(); paginator.isValidIndex(i); i++) {
            ...
            SCComponent c = new SCComponentImpl(lang("gui.add.permission.permission.title", getViewer(), permission),
                    Collections.singletonList(lang("gui.add.permission.permission.lore", getViewer())), XMaterial.PAPER,
                    slot);
            c.setListener(ClickType.LEFT, () -> InventoryController.runSubcommand(getViewer(), "rank permissions add",
                    true, rank.getName(), permission));
