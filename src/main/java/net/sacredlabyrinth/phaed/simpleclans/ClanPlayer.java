...
public class ClanPlayer implements Serializable, Comparable<ClanPlayer> {
    ...
    /**
     * Returns the player's join date to his current clan in milliseconds, 0 if not
     * in a clan
     *
     * @return the joinDate
     */
     ...
     * @return a separator delimited string with the color tags for all past clans
     *         this player has been in
     */
    @NotNull
    ...
    public String getPastClansString(String sep, @Nullable Player viewer) {
        ...
        SettingsManager settings = SimpleClans.getInstance().getSettingsManager();
        return getPastClans().stream().sorted(Collections.reverseOrder()).limit(settings.getInt(PAST_CLANS_LIMIT))
    ...
    public void addResignTime(String tag) {
        ...
        if (tag != null)
            resignTimes.put(tag, System.currentTimeMillis());
