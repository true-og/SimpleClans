...
public final class ClanManager {
    ...
    /**
     * Gets the ClanPlayer data object if a player is currently in a clan, null if
     * he's not in a clan Used for BungeeCord Reload ClanPlayer and your Clan
     */
    @Deprecated
    ...
    /**
     * Gets the ClanPlayer data object for the player, will retrieve disabled clan
     * players as well, these are players who used to be in a clan but are not
     * currently in one, their data file persists and can be accessed. their clan
     * will be null though.
     */
        ...
        if (plugin.getPermissionsManager().hasEconomy()) {
            ...
            if (plugin.getPermissionsManager().playerHasMoney(player, price)) {
                ...
                player.sendMessage(RED
                        + MessageFormat.format(lang("account.has.been.debited", player), CurrencyFormat.format(price)));
    ...
    public boolean purchaseResetKdr(Player player) {
        ...
        if (plugin.getPermissionsManager().hasEconomy()) {
            ...
            if (plugin.getPermissionsManager().playerHasMoney(player, price)) {
                ...
                player.spigot().sendMessage(new TextComponent(RED + MessageFormat
                        .format(lang("account.has.been.debited", player), CurrencyFormat.format(price))));

            } else {

                player.spigot().sendMessage(new TextComponent(RED
                        + MessageFormat.format(lang("not.sufficient.money", player), CurrencyFormat.format(price))));
                return false;
