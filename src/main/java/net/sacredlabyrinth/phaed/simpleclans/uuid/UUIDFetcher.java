 ...
 * @see <a href=
 *      "http://web.archive.org/web/20140909143249/https://gist.github.com/evilmidget38/26d70114b834f71fb3b4">Github
 *      Gist</a>
 */
public final class UUIDFetcher {
    ...
    /**
     * Fetches the UUID for a single player name. This method is a convenience
     * wrapper around the batch UUID fetching process.
     *
    ...
    private static Map<String, UUID> fetchUUIDsConcurrently(List<String> names)
            throws InterruptedException, ExecutionException
    {
    ...
    private static @Nullable UUID fetchUUID(@NotNull URL url) throws IOException {
        ...
        if (responseCode != HttpURLConnection.HTTP_OK) {
            ...
            throw new IOException(String.format("Unexpected response code: %d. Response: %s", responseCode,
                    connection.getResponseMessage()));

        }

        JsonObject response = gson.fromJson(new InputStreamReader(connection.getInputStream(), UTF_8),
                JsonObject.class);
        ...
        if (id.isJsonNull() || (response.has("status") && status.getAsString().equals("ERR"))) {
