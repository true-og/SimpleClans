...
public final class BungeeManager implements ProxyManager, PluginMessageListener {
    ...
    private static final Pattern SUBCHANNEL_PATTERN = Pattern
            .compile("SimpleClans\\|(?<subchannel>\\w+)\\|(?<version>v\\d+)\\|(?<server>.+)");
    ...
    public void onPluginMessageReceived(@NotNull String channel, @NotNull Player player, byte[] data) {
        ...
        try {
            ...
            if (!VERSION.equals(version)) {
                ...
                plugin.getLogger()
                        .severe(String.format("Unsupported channel (%s), expected version: %s", subChannel, VERSION));
    ...
    private void forwardPluginMessage(final String subChannel, final ByteArrayDataOutput message, final boolean all) {
        ...
        SimpleClans
                .debug(String.format("Forwarding message, channel %s, message %s, all %s", subChannel, message, all));
        if (!isChannelRegistered()) {
    ...
    private void sendOnBungeeChannel(ByteArrayDataOutput output) {
        ...
        Bukkit.getOnlinePlayers().stream().findAny()
                .ifPresent(player -> player.sendPluginMessage(plugin, "BungeeCord", output.toByteArray()));
