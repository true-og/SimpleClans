...
public class ClanCommands extends BaseCommand {
    ...
    public void checkFee(Player player, Clan clan) {
        ...
        ChatBlock.sendMessage(player,
                AQUA + lang("the.fee.is.0.and.its.current.value.is.1", player,
    ...
    public void setFee(Player player, Clan clan, @Name("fee") double fee) {
        ...
        if (fee > maxFee) {
            ...
            ChatBlock.sendMessage(player, RED + lang("max.fee.allowed.is.0", player, CurrencyFormat.format(maxFee)));
            return;
