...
public class ClanRoster extends Sendable {
    ...
    private void configureAndSendHeader() {
        ...
        ChatBlock.saySingle(sender,
                sm.getColored(PAGE_CLAN_NAME_COLOR) + clan.getName() + subColor + " " + lang("roster", sender) + " "
                        + headColor + Helper.generatePageSeparator(sm.getString(PAGE_SEPARATOR)));
        ChatBlock.sendBlank(sender);
        ChatBlock.sendMessage(sender,
                headColor + lang("legend", sender) + " " + sm.getColored(PAGE_LEADER_COLOR) + lang("leader", sender)
                        + headColor + ", " + sm.getColored(PAGE_TRUSTED_COLOR) + lang("trusted", sender) + headColor
                        + ", " + sm.getColored(PAGE_UNTRUSTED_COLOR) + lang("untrusted", sender));
        ChatBlock.sendBlank(sender);

        chatBlock.setFlexibility(false, true, false, true);
        chatBlock.addRow("  " + headColor + lang("player", sender), lang("rank", sender), lang("seen", sender));
