...
public class DiscordHook implements Listener {
    ...
    /**
     * Deletes channel from SimpleClans categories. If there are no channels,
     * removes category as well.
     *
     * @param channelName the channel name
    ...
    private void updateLeaderRole(@NotNull Member member, @NotNull ClanPlayer clanPlayer, DiscordAction action) {
        ...
        if (action == ADD && clanPlayer.isLeader()) {
            ...
            SimpleClans.debug(String.format("Revoked leader role from %s (%s) discord member", member.getNickname(),
                    member.getId()));
            ...
            @NotNull DiscordAction action)
    {
        ...
        if (action == ADD) {
            ...
            SimpleClans.debug(String.format("Added view permission to %s (%s) discord member", member.getNickname(),
                    member.getId()));

        } else {

            channel.getManager().removePermissionOverride(member).queue();
            SimpleClans.debug(String.format("Revoked view permission from %s (%s) discord member", member.getNickname(),
                    member.getId()));
