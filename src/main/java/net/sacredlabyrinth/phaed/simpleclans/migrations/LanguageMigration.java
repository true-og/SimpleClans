...
/**
 * Utility class that migrates old language files (yaml) to the new format
 * (properties)
 *
 * @author RoinujNosde
 *
...
public class LanguageMigration implements Migration {
    ...
    private void saveFile(File file, List<String> convertedLines) {
        ...
        try (Writer writer = new BufferedWriter(
                new OutputStreamWriter(new FileOutputStream(converted), StandardCharsets.UTF_8)))
        {
    ...
    private List<String> convertLines(List<String> lines) {
        ...
        for (String line : lines) {
            ...
            line = line.replaceFirst(":", "=").replaceAll("''''", "''").replaceAll("^'", "").replaceAll("= \'", "=")
    ...
    private String convertFileName(File file) {
        ...
        if (file.getName().equals("language.yml")) {
            ...
            fileName = file.getName().replace("language", "messages").replace("yml", "properties").replace("-", "_");
